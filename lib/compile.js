...
function compileOutputs(loader, tree, opts, sfxEntryPoint) {
  ...
  .then(function() {
    // normal bundle
    if (!sfxEntryPoint) {
      outputs.unshift('"format register";', '"deps @' + loader.transpiler + (loader.transpiler == 'babel' ? '-helpers' : '-runtime') + '";');
      return;
    }
    ...
    .then(function() {
      if (loader.transpiler == 'babel')
        return asp(fs.readFile)(path.resolve(__dirname, '../node_modules/babel-core/external-helpers.js'));
      else
        return asp(fs.readFile)(path.resolve(__dirname, '../node_modules/traceur/bin/traceur-runtime.js'));
    })
    .then(function(runtime) {
      outputs.unshift(runtime.toString());
    });
  })
  .then(function() {
    return outputs;
