...
function compileOutputs(loader, tree, opts, sfxEntryPoint) {
  ...
  .then(function() {
    // normal bundle
    if (!sfxEntryPoint) {
      // add any runtime dependencies that may be needed
      if (usesBabelHelpersGlobal)
        outputs.unshift('"deps babel-helpers";');
      if (usesTraceurRuntimeGlobal)
        outputs.unshift('"deps traceur-runtime";');
    ...
    .then(function() {
      if (opts.runtime && usesBabelHelpersGlobal)
        return getModuleSource(loader, 'babel-helpers')
...
function getModuleSource(loader, moduleName) {
