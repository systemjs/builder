...
function compileTree(loader, tree, compileOpts, outputOpts, cache) {
  ...
  // if any loads require bundle configuration, inline the configuration
  .then(function() {
    var bundleConfig;
    ordered.modules.forEach(function(m) {
      if (tree[m] && tree[m].config)
        bundleConfig = dextend(bundleConfig || {}, tree[m].config);
    });
