 ...
 * - traceResolvedConditionModules indicates if conditions modules provided to exact condition resolutions should be included in the trace.
...
Trace.prototype.getAllLoadRecords = function(canonical, traceOpts, canonicalConditionalEnv, curLoads, parentStack) {
  ...
  .then(function(load) {
    ...
    if (load) {
      ...
      return Promise.all(Trace.getLoadDependencies(load, traceOpts, canonicalConditionalEnv).map(function(dep) {
...
Trace.getConditionalResolutions = function(conditional, traceResolvedConditionModules, conditionalEnv) {
...
Trace.getLoadDependencies = function(load, traceOpts, canonicalConditionalEnv) {
  ...
  if (load.conditional) {
    var resolutions = Trace.getConditionalResolutions(load.conditional, traceOpts.traceResolvedConditionModules, canonicalConditionalEnv);

    if (traceOpts.tracePackageConfig && load.packageConfig)
      deps.push(load.packageConfig);

    if (traceOpts.traceConditionModules)
