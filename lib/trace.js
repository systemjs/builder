...
Trace.getConditionalEnv = function(builder, loads, traceOpts) {
  ...
  .then(function(canonicalConditionalEnv) {
    ...
    Object.keys(loads).forEach(function(canonical) {
      ...
      var envVariations = Trace.getConditionalResolutions(load.conditional, true, canonicalConditionalEnv).conditionalEnvVariations;
...
Trace.getConditionalResolutions = function(conditional, traceAllConditionals, conditionalEnv) {
  ...
  function booleanEnvTrace(condition) {
    var conditionObj = parseCondition(condition);
    var conditionModule = conditionObj.module;
    if (conditionModules.indexOf(conditionModule) == -1)
      conditionModules.push(conditionModule);

    if (conditionObj.negate) {
      var complement = conditionalComplement(condition);
      return conditionalEnv[complement] ? conditionalEnv[complement].indexOf(false) !== -1 : traceAllConditionals;
    }
    else {
      return conditionalEnv[condition] ? conditionalEnv[condition].indexOf(true) !== -1 : traceAllConditionals;
    }
  }
  ...
  if (conditional.branch) {
    ...
    if (parseCondition(conditional.condition).negate) {
      var complement = conditionalComplement(conditional.condition);
      conditionalEnvVariations[complement] = conditionalEnvVariations[complement] || [];
      if (conditionalEnvVariations[complement].indexOf(false) === -1)
        conditionalEnvVariations[complement].push(false);
    }
    else {
      conditionalEnvVariations[conditional.condition] = conditionalEnvVariations[conditional.condition] || [];
      if (conditionalEnvVariations[conditional.condition].indexOf(true) === -1)
        conditionalEnvVariations[conditional.condition].push(true);
    }
  ...
  // { envs: [{condition, branch},...], fallback } package environment map
    ...
    conditional.envs.forEach(function(env) {
      if (booleanEnvTrace(env.condition)) {
        branches.push(env.branch);

      if (parseCondition(env.condition).negate) {
        var complement = conditionalComplement(env.condition);
        conditionalEnvVariations[complement] = conditionalEnvVariations[complement] || [];
        if (conditionalEnvVariations[complement].indexOf(false) === -1)
          conditionalEnvVariations[complement].push(false);
      }
      else {
        conditionalEnvVariations[env.condition] = conditionalEnvVariations[env.condition] || [];
        if (conditionalEnvVariations[env.condition].indexOf(true) === -1)
          conditionalEnvVariations[env.condition].push(true);
      }
        conditionModules.push(parseCondition(env.condition).module);
      }
    ...
    if (et !== undefined && et !== false) {
      Object.keys(conditional.branches).forEach(function(branch) {
        if (et === true || et !== true && et.indexOf(branch) != -1) {
          ...
          conditionalEnvVariations[conditional.condition] = conditionalEnvVariations[conditional.condition] || [];
          conditionalEnvVariations[conditional.condition].push(branch);
          conditionModules.push(parseCondition(conditional.condition).module);
        }
      });
    }
...
Trace.getLoadDependencies = function(load, tracePackageConfig, traceRuntimePlugin, traceAllConditionals, traceConditionModules, canonicalConditionalEnv) {
  if (traceAllConditionals !== false)
    traceAllConditionals = true;
