  ...
  .then(function(load) {
    ...
    if (load)
      return Promise.all(Trace.getLoadDependencies(load, traceAllConditionals, canonicalConditionalEnv).map(function(dep) {
        return self.getAllLoadRecords(dep, traceAllConditionals, canonicalConditionalEnv, curLoads);
      }));
  })
  .then(function() {
    return curLoads;
...
Trace.prototype.getConditionLoadRecords = function(canonical, canonicalConditionalEnv, inConditionTree, curLoads) {
  var loader = this.loader;
  ...
  .then(function(load) {
    ...
    if (load)
      ...
      return Promise.all(Trace.getLoadDependencies(load, true, canonicalConditionalEnv, true).map(function(dep) {
        return self.getConditionLoadRecords(dep, canonicalConditionalEnv, true, curLoads);
      }))
      .then(function() {
        // trace non-conditions
        return Promise.all(Trace.getLoadDependencies(load, true, canonicalConditionalEnv).map(function(dep) {
          return self.getConditionLoadRecords(dep, canonicalConditionalEnv, inConditionTree, curLoads);
        }));
      });
  })
  .then(function() {
...
function getGraphEntryPoints(graph, entryPoints) {
  ...
  modules.forEach(function (moduleName) {
    Object.keys(graph.adjList[moduleName]).forEach(function(depName) {
      if (graph.adjList[depName] && Object.keys(graph.adjList[depName]).indexOf(moduleName) == -1)
