...
Trace.prototype.traceModule = function(moduleName, tracePackageConfig, traceAllConditionals, conditionalEnv, conditionsOnly) {
  ...
  .then(function(canonicalConditionalEnv) {
    if (!conditionsOnly)
      return self.getAllLoadRecords(moduleName, tracePackageConfig, traceAllConditionals, canonicalConditionalEnv, {}, []);
    else
      return self.getConditionLoadRecords(moduleName, tracePackageConfig, canonicalConditionalEnv, false, {}, []);
  })
  .then(function(loads) {
      ...
      .then(function() {
        ...
        if (load.metadata.loaderModule)
          ...
          .then(function(pluginNormalized) {
            ...
            if (pluginNormalized.indexOf('!') == -1 && getPackage(loader.packages, pluginNormalized)) {
...
Trace.prototype.getAllLoadRecords = function(canonical, tracePackageConfig, traceAllConditionals, canonicalConditionalEnv, curLoads, parentStack) {
  var loader = this.loader;
  ...
  .then(function(load) {
    ...
    if (load) {
      ...
      return Promise.all(Trace.getLoadDependencies(load, tracePackageConfig, true, traceAllConditionals, canonicalConditionalEnv).map(function(dep) {
        return self.getAllLoadRecords(dep, tracePackageConfig, traceAllConditionals, canonicalConditionalEnv, curLoads, parentStack);
      }));
    }
  })
  .then(function() {
...
Trace.prototype.getConditionLoadRecords = function(canonical, tracePackageConfig, canonicalConditionalEnv, inConditionTree, curLoads, parentStack) {
  var loader = this.loader;
  ...
  .then(function(load) {
    ...
    if (load) {
      ...
      return Promise.all(Trace.getLoadDependencies(load, tracePackageConfig, true, true, canonicalConditionalEnv, true).map(function(dep) {
        return self.getConditionLoadRecords(dep, tracePackageConfig, canonicalConditionalEnv, true, curLoads, parentStack);
      }))
      .then(function() {
        ...
        return Promise.all(Trace.getLoadDependencies(load, tracePackageConfig, true, true, canonicalConditionalEnv).map(function(dep) {
          return self.getConditionLoadRecords(dep, tracePackageConfig, canonicalConditionalEnv, inConditionTree, curLoads, parentStack);
        }));
      });
    }
  })
