...
exports.rollupTree = function(loader, tree, entryPoints, compileOpts) {
  /* 
  ...
  return Promise.all(Object.keys(optimizationGraphExternals).map(function(entryPoint) {
      ...
      plugins: [{    
        resolveId: function(id, importer, options) {
          ...
          if (externals.indexOf(resolved) != -1) {
            resolved = getAlias(loader, resolved);
            if (resolved.indexOf('#:') != -1)
              resolved = resolved.replace('#:', '/');
            // remove this pending https://github.com/rollup/rollup/issues/424
            return false;
          }
          return resolved;
        },
    ...
    .then(function(bundle) {
      ...
      if (fullTreeRollup) {
        ...
        if (compileOpts.globals)
          generateOptions.globals = compileOpts.globals;
      }
