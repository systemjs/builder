...
function getModuleName(loader, address, pluginName) {
  ...
  if (pluginName) {
    var extension = address.split(path.sep).pop();
  ...
  for (var p in loader.paths) {
    ...
    var curPath = decodeURI(url.resolve(encodeURI(loader.baseURL.replace(/\\/g, '/')), loader.paths[p])).replace(/\//g, path.sep);
    var wIndex = curPath.indexOf('*');
    if (wIndex === -1) {
      if (address === curPath) {
        curMatchLength = curPath.split(path.sep).length;
...
exports.traceExpression = function(builder, expression, sfx) {
  ...
  .then(function(tree) {
    ...
    return { 
      tree: tree, 
      entryPoints: expandedOperations.map(function(operation) {
