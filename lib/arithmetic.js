...
function parseExpression(expressionString) {
  ...
  function getNextIdentifier() {
    ...
    if (expressionString.charAt(index) === '(') {
  ...
  function getNextOperator() {
    eatWhitespace();
    return expressionString.charAt(index++); //all operators are single characters at the moment
  }
  ...
  while (index < expressionString.length) {
    var operator = getNextOperator();

    var moduleName = getNextIdentifier();

    if (typeof moduleName === 'object'){
      operations.push({
        operator: operator,
        bulkOperation: moduleName.bulkOperation
      });
    } else {
