...
function getModuleName(loader, address, pluginName) {
  ...
  if (!pathMatch) {
    if (address.substr(0, loader.baseURL.length) == loader.baseURL)
      pathMatch = address.substr(loader.baseURL.length);
    else
      throw "Unable to calculate path for " + address;
  }
  
  if (pluginName)
    pathMatch += '!' + (pluginName == extension ? '' : pluginName);
  ...
  .then(function(address) {
    ...
    return addresses.map(function(file) {
      return {
        ...
        moduleName: getModuleName(builder.loader, 'file://' + (process.platform.match(/^win/) ? '/' : '') + file, metadata.plugin),
