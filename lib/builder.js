...
function processOutputs(outputs, concatOutput, sourceMapsWithOffsets) {
  ...
  outputs.forEach(function(output) {
    ...
    source = saucy.removeSourceMaps(output.source || '');
...
exports.writeOutputFile = function(opts, outputs) {
  ...
  var createSourceMaps = opts.createSourceMaps;
  var concatOutput = [];
  ...
  return asp(mkdirp)(outPath).then(function() {
    if (createSourceMaps && (sourceMapsWithOffsets.length > 0)) {
      var sourceOutFile = path.relative(outPath, outFile) + '.map';
