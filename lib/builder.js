...
Builder.prototype.getCanonicalName = function (normalized) {
  ...
  if (plugin)
    pathMatch += '!' + this.getCanonicalName(plugin);
...
Builder.prototype.traceModule = function(moduleName) {
  ...
  .then(function(_moduleName) {
    ...
    if (thisLoad.metadata.bundle) {
      ...
      while ((curMatch = namedRegisterRegEx.exec(thisLoad.source)))
        traceTree[curMatch[3].substr(1, curMatch[3].length - 2)] = null;
    }
    else {
      return visitTree(loader.loads, moduleName, pluginLoader, function(load) {
        // copy and canonicalize the load record
        var canonicalLoad = {};
        canonicalLoad.name = self.getCanonicalName(load.name)
        ...
        for (var d in load.depMap)
          canonicalLoad.depMap[d] = self.getCanonicalName(load.depMap[d]);
  ...
  .then(function() {
    return {
      moduleName: self.getCanonicalName(moduleName),
      tree: traceTree
    };
  })
  .catch(function(e) {
